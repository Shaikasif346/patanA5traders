...
export function GridScan({
  ...
  const pushScan = (time: number) => {
    ...
    if (materialRef.current) {
      ...
      for (let i = 0; i < arr.length && i < maxScans; i++) buffer[i] = arr[i];
  ...
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (enableGyro && orientationEvent?.requestPermission) {
        try {
          ...
          // Permission can be unavailable or denied; pointer interaction still works.
        }
      }
    };
    ...
    if (scanOnClick) element.addEventListener("click", onClick);
    element.addEventListener("mouseleave", onLeave);
    return () => {
      element.removeEventListener("mousemove", onMove);
      element.removeEventListener("mouseenter", onEnter);
      element.removeEventListener("mouseleave", onLeave);
      if (scanOnClick) element.removeEventListener("click", onClick);
      if (leaveTimer) clearTimeout(leaveTimer);
    };
  ...
  useEffect(() => {
    ...
    if (material) {
      ...
      uniforms.uLinesColor.value.copy(srgbColor(linesColor));
      uniforms.uScanColor.value.copy(srgbColor(scanColor));
      uniforms.uGridScale.value = gridScale;
      uniforms.uLineStyle.value = lineStyle === "dashed" ? 1 : lineStyle === "dotted" ? 2 : 0;
      uniforms.uLineJitter.value = Math.max(0, Math.min(1, lineJitter || 0));
      uniforms.uBloomOpacity.value = Math.max(0, bloomIntensity);
      uniforms.uNoise.value = Math.max(0, noiseIntensity);
      uniforms.uScanGlow.value = scanGlow;
      uniforms.uScanOpacity.value = Math.max(0, Math.min(1, scanOpacity));
      uniforms.uScanDirection.value = scanDirection === "backward" ? 1 : scanDirection === "pingpong" ? 2 : 0;
      uniforms.uScanSoftness.value = scanSoftness;
      uniforms.uPhaseTaper.value = scanPhaseTaper;
      uniforms.uScanDuration.value = Math.max(0.05, scanDuration);
      uniforms.uScanDelay.value = Math.max(0, scanDelay);
    }
    if (bloomRef.current) {
      ...
      {showPreview && (
        <div className="gridscan__preview">
          <div className="gridscan__badge">
            {enableWebcam ? `Webcam disabled for privacy (${modelsPath ? "visual mode" : "visual mode"})` : "Visual scan mode"}
          </div>
        </div>
      )}
    </div>
